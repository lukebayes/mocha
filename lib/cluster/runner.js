...
ClusterRunner.prototype._flushWork = function() {
  ...
  console.log('inactive:', freeWorkers.length);
  ...
  if (this._workers.length === this._getFreeWorkers().length && this._pendingFiles.length === 0) {
    console.log('>>>>>>>>>>>>>>> OUTER RUNNER BEFORE END');
    this.emit('end');
    console.log('>>>>>>>>>>>>>>> OUTER RUNNER AFTER END');

    console.log('TEST EVENT COUNT:', testCount);
