...
ClusterRunner.prototype.globalProps = function() {
  // DONOTSUBMIT(lbayes): Extract BaseRunner from lib/runner.js
...
ClusterRunner.prototype._createWorker = function() {
  ...
  var workerApplication = path.normalize(path.join(__dirname, '/../../bin/_mocha'));
 ...
 */
ClusterRunner.prototype._shuffle = function(items) {
 ...
 * finished.
 */
ClusterRunner.prototype.run = function(optOnComplete) {
  this.emit('start');

  this._onComplete = optOnComplete;

  console.log('RUN TEST FILES WITH:', this.files.length);
  this._runTestFiles(this._shuffle(this.files));
};
